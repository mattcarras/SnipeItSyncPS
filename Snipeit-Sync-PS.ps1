...
function Update-SnipeItCache {
	...
	Process {
		if ($sp_entities -ne $null) {
			if ($Remove -And $Entity -is [int]) {
				$sp_entities[$Entity] = $null
				return $true
...
function Sync-SnipeItUser {
	<#
		...
		Only output debug information when creating new users. This can be useful for setting debug breakpoints.
...
function Remove-SnipeItInactiveUsers {
	...
	Process {
		...
		# Add _PURGABLE_ flag to those without any assignments
		$inactive_users = $inactive_users | Select *,@{N="_PURGABLE_"; Expression={$_.assets_count -le 0 -And $_.licenses_count -le 0 -And $_.accessories_count -le 0 -And $_.consumables_count -le 0}}
		Write-Verbose("[Remove-SnipeItInactiveUsers] Found {0} total snipe-it users that no longer exist in synced users, and of those {1} without any assignments" -f $inactive_users.Count, ($inactive_users | where {$_._PURGABLE_ -eq $true}).Count)
		if (-Not $OnlyReport) {
			...
			foreach($sp_user in $inactive_users) {
				...
				$purge_user = $null
				if ($sp_user.id -is [int]) {
					if (-Not $DontDelete -And $_._PURGABLE_ -eq $true) {
						Write-Verbose("[Remove-SnipeItInactiveUsers] PURGING inactive snipe-it user with ID [{0}], username [{1}], employee_num [{2}]" -f $sp_user.id,$sp_user.username,$sp_user.employee_num)
						$count_retry = $OnErrorRetry
						while ($count_retry -ge 0) {
						...
						$updateParams = @{}
						if ($do_set_notes -And ($_.notes -eq $null -Or $Notes -ne [System.Net.WebUtility]::HtmlDecode($_.notes).Trim())) {
							$updateParams.Add("notes", $Notes)
						}
						if ($_.activated -eq $true) {
...
function Sync-SnipeItAsset {
	<#
		...
		A default model name to lookup if the given asset's model and model_id fields are both blank or not found. Note this will never overwrite data already in Snipe-It. Either this or DefaultModelID can be given, but not both.
		
		.PARAMETER DefaultModelID
		A default model ID to lookup if the given asset's model and model_id fields are both blank or not found. Note this will never overwrite data already in Snipe-It. Either this or DefaultModel can be given, but not both.
		...
		.PARAMETER DebugOutputCreateOnly
		Only give debug output when creating new assets. This can be useful for setting debug breakpoints.
	...
	param (
		...
		[parameter(Mandatory=$true, ParameterSetName='DefaultModel')]
		[string]$DefaultModel,
		
		[parameter(Mandatory=$true, ParameterSetName='DefaultModelID')]
		[int]$DefaultModelID,
	...
	Process {
		...
		# If we don't have a valid model, check if we have a DefaultModel or DefaultModelID to use
		$using_default_model = $false
		if ($sp_model.id -isnot [int]) {
			if ($DefaultModelID -is [int]) {
				$sp_model = Get-SnipeItEntityByID $DefaultModelID "models" @passParams
				if ($sp_model.id -isnot [int]) {
					Write-Warning "[Sync-SnipeItAsset] [$UniqueID] Model [$model] and default model ID [$DefaultModelID] both returned no results"
				} else {
					$using_default_model = $true
					Write-Verbose ("[Sync-SnipeItAsset] [$UniqueID] Model [$model] not found, using default model [{0}] with model_id [{1}]" -f $sp_model.name, $sp_model.id)
				}
			} elseif (-Not [string]::IsNullOrWhitespace($DefaultModel)) {
				# Category and manaufacturer are ignored with -DontCreateIfNotFound
...
function Format-SnipeItAsset {
	<#
		.SYNOPSIS
		Formats a snipe-it asset for output to file (CSV, etc.).
	...
	Process {
		# Assumes all custom fields are found in custom_fields.
